...
export function ProductSalesReport() {
  ...
  const data = useMemo(() => SalesReportService.getProductSalesReportData(dateRange, search), [dateRange, search]);
  ...
  const columns: Column<typeof data[0]>[] = [
    ...
    { key: "totalPcs", label: "Total PCS Sold", align: "right", render: (item) => formatNumber(item.totalPcs) },
  ...
  const handleExportPDF = () => generateEnterpriseDocument({
    title: 'Product Sales Report', filters: [{ label: 'Date Range', value: dateRange.label }],
    tables: [{
      columns: exportColumns.map(c => ({ header: c.header, dataKey: c.dataKey, align: c.dataKey === 'name' ? 'left' : 'right' })),
      rows: exportData,
  ...
  return (
    <div className="space-y-6">
      <ReportFilterBar 
        ...
        onExportExcel={() => exportToExcel({ filename: 'Product_Sales.xlsx', data: exportData, columns: exportColumns })}
        onExportCSV={() => exportToCSV({ filename: 'Product_Sales.csv', data: exportData, columns: exportColumns })}
      />
